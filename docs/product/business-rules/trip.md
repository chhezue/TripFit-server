...
| :--- | :--- | :--- | :--- | :--- |
| **BR-TRIP-001** | 여행방 생성 필수 정보 | 방장이 여행방을 생성할 때 | 여행방 이름(최대 **15자**), 여행 희망 기간(탐색 범위), 희망 여행 일수(n박 m일), 예상 참여 인원 필수. **여행지**는 `trip.destination`에 저장 (입력 UI는 선택 가능) | 필수 값 누락·이름 16자 이상 시 400 |
