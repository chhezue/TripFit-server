...
| **홈 Pin** | `trip_member.is_pinned` — **진행중인 여행 캐러셀**에서 Pin 우선 노출; `end_range` 경과 시 자동 해제 (정책서) |
...
| 화면 | 데이터·액션 | BR | wave |
|------|-------------|-----|------|
| 홈 목록 | `GET /trips`: pin → `updatedAt DESC` | D5 확정 | 2 |
...
| Pin 토글 | trip_member.is_pinned | BR-USER (홈 정렬 D5) | 2 |
...
| **Pin·내보내기** | Pin MVP In (D5). 내보내기 UI — API [#20](https://github.com/Central-MakeUs/TripFit-server/issues/20) (wave 2 Out) |
