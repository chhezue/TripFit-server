...
**0. 문서·구현 정합 (최우선)** — 스펙·결정·문서 간 값·계약이 어긋나면 **구현하지 말고 사용자에게 질문**. 상세: `.cursor/rules/harness-workflow.mdc` ⛔ STOP.  
**Wave Must/Nice/Out** — 이슈 `## Must Have`·스펙 `MVP: In scope`만으로 Must를 단정하지 않음. SSOT: Wave Backlog(`#29`~`#32`) · `.cursor/rules/harness-wave.mdc`.

기획·검증 기준을 먼저 고정하고, 그에 맞춰 구현합니다. **계획 축**은 `docs/product/development-wave.md` (운영) · `docs/product/waves.md` (요약, wave 1~4), **기획**은 `docs/product/`, **기능 설계**는 `docs/specs/`, **아키텍처 선택**은 `docs/decisions/`에 둡니다. DB·인증·다파일 변경 시 스펙 필수 (`harness-workflow` 규칙). 구현 후 `./gradlew test`와 PR·CI로 검증합니다.
...
- Java 주석: Swagger·`@Schema`와 중복 금지 — Service·client는 단계(`// 1.`) + Why 병행, Controller는 권한·검증만 — `.cursor/rules/spring-boot-java.mdc` Comments 절
- 범위 밖 리팩터링·포맷 변경 금지 — 요청된 작업만 수정
- 커밋은 사용자가 명시적으로 요청할 때만
- 커밋 요청 시 주제별 **최대 3개**로 분할 (상세: `.github/CONTRIBUTING.md`, `.cursor/rules/harness-workflow.mdc`)
...
- **Wave Must/Nice/Out:** Backlog(`#29`~`#32`) 없이 단정 금지 — `harness-wave.mdc`
- **DB:** 상용 보존 데이터 없음 → Flyway/SQL 마이그레이션 **작성 금지**. 엔티티 최신본 + `ddl-auto`, 필요 시 DB 리셋 (`.cursor/rules/harness-workflow.mdc`)
- **레거시:** 현행 Approved 정책과 다른 코드·호환 레이어는 **삭제** (구 클라/DB 호환 금지)
- **ERD:** 고정이 아님 — 스키마 개선을 **적극 제안**, 승인 시 엔티티+`erd.md`만 최신화 (`harness-follow-up.mdc`)
...
| [`docs/product/waves.md`](docs/product/waves.md) | **계획·우선순위** — wave 1~4 요약 |
| [`docs/product/development-wave.md`](docs/product/development-wave.md) | **Wave 운영·판단·GitHub Backlog** SSOT |
...
에이전트는 Superpowers process skills(`using-superpowers`, `brainstorming`, `systematic-debugging`, `verification-before-completion` 등)를 사용한다.

- **문서·스펙·Approved 계약**은 `.cursor/rules/harness-workflow.mdc` ⛔가 Superpowers보다 **우선**
- **기능 스펙 작성**은 플러그인이 아니라 `.cursor/skills/specify/`가 SSOT
- 스킬 트리거·우선순위: `.cursor/rules/superpowers-workflow.mdc` · [`.cursor/README.md`](.cursor/README.md)
